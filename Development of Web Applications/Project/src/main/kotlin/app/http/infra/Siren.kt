...
data class ActionModel(
    ...
    val method: String,
    val type: String,
    val needsAuthentiaction : Boolean,
...
class SirenBuilderScope<T>(val properties: T) {
    ...
    fun action(name: String, href: URI, method: HttpMethod, type: String, needsAuthentiaction : Boolean,block: ActionBuilderScope.() -> Unit) {
        val scope = ActionBuilderScope(name, href, method, type,needsAuthentiaction)
        scope.block()
...
class EntityBuilderScope<T>(val properties: T,val rel: List<String>) {
    private val links = mutableListOf<LinkModel>()
    fun link(href: URI, rel: LinkRelation){
        links.add(LinkModel(listOf(rel.value),href.toASCIIString()))
    }
...
class ActionBuilderScope(
    ...
    private val needsAuthentiaction: Boolean
) {
    ...
    fun playField(name: String){
        fields.add(FieldModel(name,"playInfo"))
    }

    fun build() = ActionModel(name, href.toASCIIString(), method.name, type, needsAuthentiaction,fields)
}
